...
export async function notifyNewOrder(payload: NewOrderPayload) {
  try {
    ...
    }).catch(err => console.error("Error calling n8n new-order webhook:", err));
  } catch (error) {
...
export async function notifyOrderStatusChange(payload: OrderStatusPayload) {
  try {
    ...
    fetch(url, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }).catch(err => console.error("Error calling n8n order-status webhook:", err));
  } catch (error) {
...
export async function notifyMarketing(payload: MarketingPayload) {
  try {
    ...
    fetch(url, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }).catch(err => console.error("Error calling n8n marketing webhook:", err));
  } catch (error) {
